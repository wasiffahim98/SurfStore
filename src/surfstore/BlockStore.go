...
// For every block in BlockMap
// Compute blockIndex
// Check if this is within inst.LowerIndex & inst.upperIndex
// If yes, call BlockStore.PutBlock on the inst.DestAddr
...
func (bs *BlockStore) MigrateBlocks(inst MigrationInstruction, succ *bool) error {
	...
	for key, element := range bs.BlockMap {
		currIndexVal := HashMod(key, bs.RingSize)
		if inst.LowerIndex < inst.UpperIndex {
			if currIndexVal >= inst.LowerIndex && currIndexVal <= inst.UpperIndex {
				e = conn.Call("BlockStore.PutBlock", element, succ)
				if e != nil {
					return e
				}
			}
		} else if inst.LowerIndex > inst.UpperIndex {
			if currIndexVal >= inst.LowerIndex || currIndexVal <= inst.UpperIndex {
				e = conn.Call("BlockStore.PutBlock", element, succ)
				if e != nil {
					return e
